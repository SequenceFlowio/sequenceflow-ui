...
export function SupportStory() {
  return (
    <div className="so-story" id="features">
      <ScrollTilt selector=".so-order-card, .so-document" anchor=".so-visual" />
      ...
      <Reveal as="section" className="so-feature so-feature--order">
        ...
        <div className="so-visual so-order-visual" aria-label="Voorbeeld: een klantvraag, de opgehaalde bestelling en het antwoordconcept"><div className="so-floating-question"><Mail size={17} /><span>“Waar blijft mijn bestelling?”</span></div><div className="so-order-card"><div className="so-card-heading"><div className="so-icon"><Package size={21} /></div><div><strong>Bestelling #4521</strong><small>Opgehaald uit je webshop</small></div><span className="so-state">Onderweg</span></div><div className="so-shipping-track"><span className="is-done"><Check size={12} /></span><i /><span className="is-done"><Check size={12} /></span><i /><span /></div><div className="so-track-labels"><span>Besteld</span><span>Verzonden</span><span>Bezorgd</span></div><div className="so-order-note"><span>Vervoerder</span><strong>PostNL</strong></div></div><div className="so-context-link"><ArrowDown size={18} /><span>Gebruikt in het antwoord</span></div><div className="so-knowledge-answer"><SequenceMark size={32} title="" /><p>“Hoi Jan, je bestelling #4521 is verzonden en onderweg met PostNL. Via de track &amp; trace zie je waar je pakket nu is.”</p></div></div>
      </Reveal>
