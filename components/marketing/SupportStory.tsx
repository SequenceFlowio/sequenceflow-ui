import { ArrowDown, ArrowUpRight, Check, FileText, Mail, Package, ShieldCheck } from "lucide-react";
  ...
  return (
    <div className="so-story" id="features">
      ...
      <Reveal as="section" className="so-feature so-feature--reverse">
        <div className="so-feature-copy"><span className="so-label">02 / JOUW KENNIS</span><h2>Jouw beleid.<br />In ieder concept.</h2><p>Voeg je retourbeleid, verzendinformatie en productkennis toe. Support gebruikt relevante passages om een antwoord voor te bereiden dat bij jouw bedrijf past.</p><a href="#voorbeelden" className="mk-text-link">Probeer de voorbeelden <ArrowUpRight size={15} /></a></div>
      ...
      <Reveal as="section" className="so-feature so-control">
        <div className="so-feature-copy"><span className="so-label">03 / MENSELIJKE CONTROLE</span><h2>Een extra collega.<br />Jij blijft aan het stuur.</h2><p>Lees het concept, pas het aan of neem de vraag zelf over. Je begint met menselijke goedkeuring. Automatisch versturen stel je later bewust in, met je eigen voorwaarden en verzendmomenten.</p><span className="so-inline-proof"><ShieldCheck size={18} /> Menselijke controle is de standaard</span></div>
      ...
      <div className="so-feature-pair">
        <Reveal as="article" className="so-small-feature"><span className="so-label">04 / ANTWOORDSTIJL</span><h2>Maak van een correctie<br />een duidelijke afspraak.</h2><p>Support kan verbeteringen voorstellen op basis van aanpassingen aan antwoorden. Jij bepaalt welke afspraken actief worden.</p><div className="so-rule-demo"><span className="so-label">VOORBEELD VAN EEN LEERVOORSTEL</span><p><s>Geachte klant,</s><span>Hoi Sanne,</span></p><div><FileText size={17} /><span>Spreek klanten aan met hun voornaam.</span><span className="so-state">Ter beoordeling</span></div></div></Reveal>
        <Reveal as="article" className="so-small-feature" delay={100}><span className="so-label">05 / JE BESTAANDE MAILBOX</span><h2>Een nieuwe collega.<br />Je vertrouwde supportadres.</h2><p>Of je nu op bol, Shopify of WooCommerce verkoopt: klantvragen die in je gekoppelde mailbox binnenkomen, kan Support voorbereiden. Je webshop koppel je apart voor actuele bestelgegevens.</p><div className="so-mail-route"><div><Mail size={22} /><strong>Jouw mailbox</strong><small>Inkomende klantvraag</small></div><span aria-hidden>→</span><div><SequenceMark size={42} title="" /><strong>Support One</strong><small>Concept ter controle</small></div></div><p className="so-small-note">Ontvangen via forwarding of IMAP · versturen via SMTP</p></Reveal>
      </div>
