  ...
  return (
    <GuideMockWindow address="dev.shopify.com">
      ...
      <div className="commerce-guide-empty">
        ...
        <strong>{nl ? "Maak je Support-app" : "Create your Support app"}</strong>
...
function InstallVisual({ nl }: { nl: boolean }) {
  return (
    <GuideMockWindow address="admin.shopify.com">
      ...
      <div className="commerce-guide-permission"><LockKeyhole size={20} /><div><strong>{nl ? "Support toegang geven" : "Give Support access"}</strong><small>{nl ? "Alleen orders lezen en gecontroleerd wijzigen" : "Only read and safely update orders"}</small></div></div>
...
function CredentialsVisual({ nl }: { nl: boolean }) {
  return (
    <GuideMockWindow address="dev.shopify.com">
      ...
      <div className="commerce-guide-verified"><ShieldCheck size={15} />{nl ? "Support controleert de rest" : "Support verifies the rest"}</div>
    </GuideMockWindow>
  );
}
...
export default function ShopifySetupGuide({ language, open, onClose }: ShopifySetupGuideProps) {
  ...
  const slides: CommerceGuideSlide[] = nl ? [
    { title: "Maak één Shopify-app", description: "Open het Shopify Dev Dashboard van de webshopeigenaar, kies App maken en noem de app Support.", note: "Dit is een eenmalige pilotstap per webshop.", visual: <CreateAppVisual nl /> },
    { title: "Geef minimale toegang", description: "Voeg alleen read_orders en write_orders toe. Kies webhookversie 2026-07 en activeer bij beschermde ordergegevens alleen Email.", note: "Support vraagt geen klantnamen, adressen of volledige orderhistorie op.", visual: <AccessVisual nl /> },
    ...
    { title: "Neem drie gegevens over", description: "Kopieer het .myshopify.com-domein, de Client ID en het Client secret naar Support. Klik daarna op Opslaan en controleren.", note: "Daarna beheert Support tokens, controles, webhooks en synchronisatie automatisch.", visual: <CredentialsVisual nl /> },
  ] : [
    { title: "Create one Shopify app", description: "Open the store owner's Shopify Dev Dashboard, choose Create app, and name it Support.", note: "This is a one-time pilot step for each store.", visual: <CreateAppVisual nl={false} /> },
    { title: "Grant minimal access", description: "Add only read_orders and write_orders. Select webhook version 2026-07 and enable only Email under protected order data.", note: "Support doesn't request customer names, addresses, or full order history.", visual: <AccessVisual nl={false} /> },
    ...
    { title: "Copy three credentials", description: "Copy the .myshopify.com domain, Client ID, and Client secret into Support. Then select Save and verify.", note: "Support then manages tokens, checks, webhooks, and synchronization automatically.", visual: <CredentialsVisual nl={false} /> },
  ];
