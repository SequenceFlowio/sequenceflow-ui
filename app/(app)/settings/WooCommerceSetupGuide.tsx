...
function RestApiVisual({ nl }: { nl: boolean }) {
  return (
    <GuideMockWindow address="shop.nl/wp-admin">
      ...
      <div className="commerce-guide-empty"><div className="commerce-guide-empty__icon"><KeyRound size={22} /></div><strong>{nl ? "Maak een aparte Support-sleutel" : "Create a separate Support key"}</strong><small>{nl ? "Zo kun je toegang later altijd intrekken." : "You can revoke access at any time."}</small></div>
    </GuideMockWindow>
  );
}
...
function GenerateKeyVisual({ nl }: { nl: boolean }) {
  return (
    <GuideMockWindow address="shop.nl/wp-admin">
      ...
      <div className="commerce-guide-field"><small>{nl ? "Beschrijving" : "Description"}</small><div className="commerce-guide-select">Support</div></div>
...
function CopyKeysVisual({ nl }: { nl: boolean }) {
  ...
  return (
    <GuideMockWindow address="shop.nl/wp-admin">
      ...
      <div className="commerce-guide-verified"><ShieldCheck size={15} />{nl ? "Support test toegang en webhooks" : "Support tests access and webhooks"}</div>
    </GuideMockWindow>
  );
}
...
export default function WooCommerceSetupGuide({ language, open, onClose }: WooCommerceSetupGuideProps) {
  ...
  const slides: CommerceGuideSlide[] = nl ? [
    { title: "Open WooCommerce-instellingen", description: "Log in op het WordPress-beheer van je webshop en open WooCommerce → Instellingen.", note: "Je maakt de sleutel in je eigen webshop; Support krijgt nooit je WordPress-wachtwoord.", visual: <OpenSettingsVisual nl /> },
    { title: "Ga naar de REST API", description: "Open Geavanceerd → REST API en kies Sleutel toevoegen. Maak een aparte sleutel voor Support.", note: "Werkt de API niet, controleer dan onder Instellingen → Permalinks dat niet 'Plain' is geselecteerd.", visual: <RestApiVisual nl /> },
    { title: "Kies Read/Write", description: "Gebruik Support als beschrijving, kies een beheerder met toegang tot orders en zet Rechten op Read/Write. Genereer daarna de sleutel.", note: "Write-toegang is nodig voor goedgekeurde annuleringen; zonder admin-goedkeuring voert Support niets uit.", visual: <GenerateKeyVisual nl /> },
    { title: "Kopieer de sleutel direct", description: "Neem je webshop-URL, Consumer key en Consumer secret over in Support en klik op Opslaan en controleren.", note: "WooCommerce toont het secret maar één keer. Daarna controleert Support toegang en webhooks automatisch.", visual: <CopyKeysVisual nl /> },
  ] : [
    { title: "Open WooCommerce settings", description: "Sign in to your store's WordPress admin and open WooCommerce → Settings.", note: "You create the key inside your own store; Support never receives your WordPress password.", visual: <OpenSettingsVisual nl={false} /> },
    { title: "Open the REST API", description: "Go to Advanced → REST API and select Add key. Create a separate key for Support.", note: "If the API doesn't work, check Settings → Permalinks and make sure Plain isn't selected.", visual: <RestApiVisual nl={false} /> },
    { title: "Select Read/Write", description: "Use Support as the description, select an administrator with order access, and set Permissions to Read/Write. Then generate the key.", note: "Write access is required for approved cancellations; Support never acts without admin approval.", visual: <GenerateKeyVisual nl={false} /> },
    { title: "Copy the key immediately", description: "Copy your store URL, Consumer key, and Consumer secret into Support and select Save and verify.", note: "WooCommerce shows the secret only once. Support then verifies access and webhooks automatically.", visual: <CopyKeysVisual nl={false} /> },
  ];
