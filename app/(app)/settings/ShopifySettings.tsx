...
export default function ShopifySettings() {
  ...
  const labels = nl ? {
    ...
    approvalDescription: "Support mag een annulering voorstellen. Een admin moet altijd goedkeuren voordat Shopify iets uitvoert.",
    max: "Maximumbedrag",
    ...
    policyDisabledText: "Support voert geen Shopify-annuleringen uit.",
    ...
    setupDescription: "Maak eenmalig een app voor deze webshop. Daarna beheert Support de verbinding en tokenvernieuwing automatisch.",
    ...
    automaticCheckText: "Support controleert de shoptoegang, orderrechten en webhookversie voordat de koppeling actief wordt.",
    ...
    approvalDescription: "Support may propose a cancellation. An admin must always approve before Shopify executes anything.",
    max: "Maximum amount",
    ...
    policyDisabledText: "Support won't execute Shopify cancellations.",
    ...
    setupDescription: "Create one app for this store once. Support then manages the connection and token renewal automatically.",
    openDashboard: "Open Shopify Dev Dashboard",
    ...
    automaticCheckText: "Support verifies store access, order scopes, and the webhook version before activating the connection.",
      ...
      <div style={{ padding: "16px 17px", display: "grid", gap: 16 }}>
        {connectedState ? (
          <>
            ...
            {manageOpen ? <div style={{ borderTop: "1px solid var(--border)", paddingTop: 15, display: "grid", gap: 14 }}><div><p style={{ margin: 0, fontSize: 12, fontWeight: 800 }}>{labels.connectionSettings}</p><p style={{ margin: "3px 0 0", color: "var(--muted)", fontSize: 11 }}>{labels.connectionDescription}</p></div>{connectionForm}<div style={{ display: "flex", gap: 8, flexWrap: "wrap", borderTop: "1px solid var(--border)", paddingTop: 13 }}><button disabled={controlsDisabled} style={commerceButtonStyle} onClick={() => run(connection?.status === "paused" ? "resume" : "pause", () => fetch("/api/integrations/shopify", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: connection?.status === "paused" ? "active" : "paused" }) }), { tone: "success", title: connection?.status === "paused" ? labels.resume : labels.pause, text: connection?.status === "paused" ? labels.webhooksActive : labels.webhooksPaused })}><Pause size={14} />{connection?.status === "paused" ? labels.resume : labels.pause}</button><button disabled={controlsDisabled} style={{ ...commerceButtonStyle, marginLeft: "auto", color: "var(--tone-danger)" }} onClick={() => { if (window.confirm(nl ? "Shopify ontkoppelen? De live koppeling en orderdata worden verwijderd." : "Disconnect Shopify? The live connection and order data will be removed.")) void run("delete", () => fetch("/api/integrations/shopify", { method: "DELETE" }), { tone: "success", title: labels.disconnect, text: nl ? "Shopify is veilig ontkoppeld." : "Shopify was safely disconnected." }); }}><Unplug size={14} />{labels.disconnect}</button></div></div> : null}
          </>
        ) : (
          <>
