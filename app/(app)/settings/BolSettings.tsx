      ...
      <div style={{ padding: "16px 17px", display: "grid", gap: 16 }}>
        {active ? (
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 10, padding: "11px 12px", border: "1px solid rgba(199,245,111,.3)", borderRadius: 8, background: "rgba(199,245,111,.1)", color: "var(--tone-success)" }}><ShieldCheck size={17} style={{ flex: "none" }} /><div><strong style={{ display: "block", fontSize: 11 }}>{nl ? "Read-only in v1" : "Read-only in v1"}</strong><p style={{ margin: "2px 0 0", fontSize: 10, lineHeight: 1.5 }}>{nl ? "Support leest context, maar annuleert, retourneert, verzendt of wijzigt nooit voorraad via bol.com." : "Support reads context but never cancels, returns, ships, or changes inventory through bol.com."}</p></div></div>
            {notice ? <FeedbackNotice notice={notice} closeLabel={nl ? "Sluiten" : "Close"} onClose={() => setNotice(null)} /> : null}
