  ...
  return <div className="settings-stack">
    ...
    <Section icon={<Users size={18} />} title={t.settings.teamMembers} description={nl ? "Wie met Support One meewerkt." : "Who works with Support One."} action={<div className="settings-actions"><span className="settings-status">{team.seats.used} / {team.seats.limit ?? "∞"} {nl ? "plaatsen" : "seats"}</span>{team.canManage && !showInvite && !atLimit ? <button className="settings-btn primary" onClick={() => { setShowInvite(true); setInviteError(null); }}><UserPlus size={14} />{t.settings.teamInviteTitle}</button> : null}</div>}>
      <div className="settings-list">{sortedMembers.map((member) => {
        ...
        const reason = member.isCurrentUser ? (nl ? "Je kunt je eigen account niet verwijderen." : "You cannot remove your own account.") : (nl ? "Er moet minimaal één beheerder blijven." : "At least one admin must remain.");
