...
function mutationMessage(status: number, error: unknown, fallback: string, nl: boolean) {
  ...
  if (status === 403) return nl ? "Alleen een beheerder kan dit wijzigen." : "Only an admin can make this change.";
  ...
  if (message.includes("one admin")) return nl ? "Er moet minimaal één beheerder blijven." : "At least one admin must remain.";
  return fallback;
}
...
export default function TeamSettings() {
  ...
  return <div className="settings-stack">
    ...
    {!team.canManage ? <Notice tone="info" title={nl ? "Alleen-lezen" : "Read only"}>{nl ? "Alleen beheerders kunnen teamleden uitnodigen of verwijderen." : "Only admins can invite or remove team members."}</Notice> : null}
    <Section icon={<Users size={18} />} title={t.settings.teamMembers} description={nl ? "Wie met Support One meewerkt." : "Who works with Support One."} action={<div className="settings-actions">{team.seats.limit != null ? <span className="settings-status">{team.seats.used} / {team.seats.limit} {nl ? "plaatsen" : "seats"}</span> : null}{team.canManage && !showInvite && !atLimit ? <button className="settings-btn primary" onClick={() => { setShowInvite(true); setInviteError(null); }}><UserPlus size={14} />{t.settings.teamInviteTitle}</button> : null}</div>}>
      <div className="settings-list">{sortedMembers.map((member) => {
        ...
        return <div className="settings-list-row" key={member.user_id}><div className="settings-row-main"><span className="settings-avatar">{initials(member)}</span><div className="settings-row-copy"><div className="settings-actions"><strong>{member.name || member.email || "–"}</strong>{member.isCurrentUser ? <span className="settings-status success">{nl ? "Jij" : "You"}</span> : null}{member.status === "invited" ? <span className="settings-status warning">{nl ? "Uitgenodigd" : "Invited"}</span> : null}</div><span>{member.email ?? "–"} · {member.role === "admin" ? t.settings.teamRoleAdmin : t.settings.teamRoleAgent}</span></div></div>{team.canManage && !protectedMember ? <button type="button" className="settings-btn danger icon" title={t.settings.teamRemove} aria-label={`${t.settings.teamRemove}: ${member.email}`} onClick={() => { setRemoveError(null); setRemoveMember(member); }}><Trash2 size={14} /></button> : null}</div>;
      })}</div>
