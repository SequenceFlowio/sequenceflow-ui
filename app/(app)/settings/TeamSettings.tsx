...
export default function TeamSettings() {
  ...
  return <div className="settings-stack">
    ...
    <Section icon={<Users size={18} />} title={t.settings.teamMembers} description={nl ? "Beheer wie toegang heeft tot deze Support-workspace." : "Manage who has access to this Support workspace."} action={<div className="settings-actions"><span className="settings-status success">{team.members.length} {nl ? "leden" : "members"}</span>{team.canManage && !showInvite && !atLimit ? <button className="settings-btn primary" onClick={() => { setShowInvite(true); setInviteError(null); }}><UserPlus size={14} />{t.settings.teamInviteTitle}</button> : null}</div>}>
