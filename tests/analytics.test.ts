...
test("analytics UI and APIs expose partial failures and honest samples", () => {
  ...
  assert.doesNotMatch(page, /analytics-status/);
...
test("commerce intelligence cache is tenant-bound, aggregate-only, and shown separately from pain points", () => {
  ...
  // Inzicht toont alleen de vier kernpanelen; bestelgegevens staan op /commerce.
  assert.doesNotMatch(page, /\/api\/analytics\/commerce-intelligence/);
});
