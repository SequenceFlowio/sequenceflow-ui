...
test("archive routes are tenant-bound and permanent deletion requires archive", () => {
  ...
  assert.match(inboxPage, /appFetch\("\/api\/tickets\/bulk-archive"/);
...
test("spam feedback is server-controlled and never mutates the provider mailbox", () => {
  ...
  assert.ok(pipelineSource.indexOf("checkAiAnswerLimit(input.tenantId)") < pipelineSource.indexOf("openai.chat.completions.create"));
});
